"#!/bin/bash
# Write .cargo/config.toml for xtensa-esp32s3-espidf (std)
cat > /workspaces/Drn/.cargo/config.toml << 'EOF'
# .cargo/config.toml
...
# Write Cargo.toml with correct esp-idf-svc features
cat > /workspaces/Drn/Cargo.toml << 'EOF'
[package]
...
echo "Configs written successfully. Ready for cargo build --release."
"